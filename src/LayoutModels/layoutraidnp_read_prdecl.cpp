...
/**
 * @file	layoutraidnp_read_prdecl.cpp
 * @brief	Functions for read requests for RAID-NP with parity declustering.
 */

#include "LayoutModels/layoutraidnp.hpp"

void
LayoutRAIDNP::__READ_PARDEC_SREQOPT_PARITYRD (
...
void
LayoutRAIDNP::__READ_PARDEC_SREQOPT (
	OGSS_Ulong				idxRequest,
	std::vector < OGSS_Ulong >	& subrequests)
{
	std::vector < REQ > tmp_a = std::vector < REQ > ();
...
void
LayoutRAIDNP::__READ_PARDEC_PARITYRD (
	OGSS_Ulong				idxRequest,
	std::vector < OGSS_Ulong >	& subrequests)
{
