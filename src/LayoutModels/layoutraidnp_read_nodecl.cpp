...
/**
 * @file	layoutraidnp_read_nodecl.cpp
 * @brief	Functions for read requests for RAID-NP without parity declustering.
 */

#include "LayoutModels/layoutraidnp.hpp"

void
LayoutRAIDNP::__READ_NODEC_SREQOPT_PARITYRD (
...
void
LayoutRAIDNP::__READ_NODEC_SREQOPT (
	OGSS_Ulong				idxRequest,
	std::vector < OGSS_Ulong >	& subrequests)
{
	std::vector < REQ > tmp_a = std::vector < REQ > ();
...
void
LayoutRAIDNP::__READ_NODEC_PARITYRD (
	OGSS_Ulong				idxRequest,
	std::vector < OGSS_Ulong >	& subrequests)
{
